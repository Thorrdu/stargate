        ...
        */
        ...
        for( $phase = 1 ; $phase <= 14 ; $phase++ )
        {
            ...
            foreach($attackForces as $key => $forceUnit)
            {
                ...
                {
                    $attackForces[$key]['shield_left'] += (($forceUnit['shield']-$forceUnit['shield_left']) * 0.1) * (12-$phase);
                    $forceUnit['shield_left'] = $attackForces[$key]['shield_left'];
                }
            ...
            foreach($defenceForces as $key => $forceUnit)
            {
                //Regen shield
                if($forceUnit['shield_left'] < $forceUnit['shield'] && $phase > 1 && $phase < 12){
                    $defenceForces[$key]['shield_left'] += (($forceUnit['shield']-$forceUnit['shield_left']) * 0.1) * (12-$phase);
                    $forceUnit['shield_left'] = $defenceForces[$key]['shield_left'];
                }
        ...
        $battleType = 'descriptionwithout';
        if(!empty($ruinFieldString))
            $battleType = 'descriptionwith';
        Alert::create([
            'type' => 'battle',
            'news_fr' => trans('alert.battle.'.$battleType, ['player1' => $this->sourcePlayer->user_name,'player2' => $this->destinationPlayer->user_name,'galaxy' => $this->destinationColony->coordinates->galaxy], 'fr'),
            'news_en' => trans('alert.battle.'.$battleType, ['player1' => $this->sourcePlayer->user_name,'player2' => $this->destinationPlayer->user_name,'galaxy' => $this->destinationColony->coordinates->galaxy], 'en'),
