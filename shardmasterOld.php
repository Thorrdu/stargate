...
Prochains timers: "; //(complete list: <#555019107523887124>)
    ...
    $discord->on('ready', function ($discord) {
        ...
        echo "ShardMaster is online/refreshed. Now listening to connected discords.", PHP_EOL;

        global $messageId, $waitingForMessage, $autoTimer, $messageIdTop, $messageIdThirdTimer, $waitingForThirdMessage, $messageIdSecondaryTop, $waitingForMessageTop, $waitingForMessageSecondaryTop, $waitingForSecondaryMessage, $messageIdSecondayTimer;
        ...
        ])->then(function ($messageHMen) use ($channel) {

            global $waitingForMessage, $messageId;

            $lastMess = $messageHMen->first();

            if (!empty($lastMess) && $lastMess->author->id == 517045134714470400) {

                $messageId = $lastMess->id;
            } else {

                $channel->sendMessage(getTimers());
        ...
        $channel->getMessageHistory([
            ...
            global $waitingForMessageTop, $messageIdTop;

            $lastMess = $messageHMen->first();

            if (!empty($lastMess) && $lastMess->author->id == 517045134714470400) {

                $messageIdTop = $lastMess->id;
            } else {

                $channel->sendMessage(getTop(0));

                $waitingForMessageTop = true;
            }
        }, function ($e) {
        ...
        //pttibotid 517045134714470400

        //ThorrId   125641223544373248

        //HiroId   280278648517296128

        //EnderBot  280726849842053120
        ...
        if ($upToDate) {
            ...
            $discord->loop->addPeriodicTimer(1, function () use ($discord) {
                ...
                global $realTimers, $shardsinfos, $autoTimer, $db, $externalNotifs;


                foreach ($shardsinfos as $id => $infos) {
            ...
            Prochain timer du Shard " . $id . ": ";

                        $guild = $discord->guilds->get('id', $infos['guild_id']);

                        $channel = $guild->channels->get('id', $infos['premium_channel_id']);


                        if (is_a($infos['current_timer'], 'DateTime')) {

                            if (is_a($infos['current_timer'], 'DateTime')) {

                                $timeNow = new DateTime('NOW');

                                if ($timeNow < $infos['current_timer']) {

                                    $interval = $timeNow->diff($infos['current_timer']);

                                    $secondsToShow = $seconds = (int) $interval->format("%s");

                                    if ($seconds < 10)

                                        $secondsToShow = "0" . $seconds;

                                    $minutesToShow = $minutes = (int) $interval->format("%i");

                                    if ($minutes < 10)

                                        $minutesToShow = "0" . $minutes;

                                    $timerToShow = $minutesToShow . "m" . $secondsToShow . "s";
                                    ...
                                    if ($minutes < 1 && $seconds <= 30) {
                                        ...
                                        if ($infos['warning_flag']) {

                                            $role = $guild->roles->get("name", "Notif");

                                            $mentions = " <@&" . $role->id . "> ";

                                            if (isset($realTimers[$infos['premium_channel_id']]['message_mention_id']) && !empty($realTimers[$infos['premium_channel_id']]['message_mention_id']))

                                                $channel->deleteMessage($realTimers[$infos['premium_channel_id']]['message_mention_id']);

                                            echo PHP_EOL . $timeNow->format("H:i:s") . " Shard " . $id . " dans moins de 30 secondes" . $mentions;

                                            $channel->sendMessage(" Shard " . $id . " dans moins de 30 secondes" . $mentions)->then(function ($messageHMen) use ($infos) {

                                                global $realTimers;

                                                $realTimers[$infos['premium_channel_id']]['message_mention_id'] = $messageHMen->id;
                                            }, function ($e) {
                                            ...
                                            if (!empty($externalNotifs[$id])) {

                                                foreach ($externalNotifs[$id] as $chanToNotif) {

                                                    $externalGuild = $discord->guilds->get('id', $chanToNotif['guild_id']);

                                                    $externalChannel = $externalGuild->channels->get('id', $chanToNotif['channel_id']);

                                                    $externalRole = $externalGuild->roles->get("name", "Notif");

                                                    $mentions = " <@&" . $externalRole->id . "> ";

                                                    $externalChannel->sendMessage(" Shard " . $id . " dans moins de 30 secondes" . $mentions);
                                                }
                                            }
                                        }
                                    } elseif (!$infos['warning_flag']) {

                                        $shardsinfos[$id]['warning_flag'] = true;
                                    }

                                    $timerToShow = $timerToShow . " || " . $infos['current_timer']->format("H:i:s");
                                } else {
                        ...
                        if ($autoTimer) {
                            ...
                            if ($realTimers[$infos['premium_channel_id']]['reload'] && $realTimers[$infos['premium_channel_id']]['reload_date'] < $timeNow) {

                                if (!empty($realTimers[$infos['premium_channel_id']]['message_id']))

                                    $channel->deleteMessage($realTimers[$infos['premium_channel_id']]['message_id']);

                                $channel->sendMessage($replyContent)->then(function ($messageH) use ($infos) {

                                    global $realTimers;


                                    $realTimers[$infos['premium_channel_id']]['message_id'] = $messageH->id;
                                }, function ($e) {

                                    print_r($e->getMessage());
                                });

                                $realTimers[$infos['premium_channel_id']]['reload'] = false;
                            } else {

                                $channel->patchMessage($replyContent, $realTimers[$infos['premium_channel_id']]['message_id']);
                            }
                        }
                    }
                }
